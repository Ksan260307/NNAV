...
class Stage:
    ...
    def unlocked(self, vocab: int, turns: int, understanding: float) -> bool:
        return (vocab >= self.need_vocab and turns >= self.need_turns
                and understanding >= self.need_understanding)


STAGES: List[Stage] = [
    Stage(0, "EGG", "卵 / 起動直後", 0, 0, 0.0,
          GenParams(allow_fact=True, min_len=1, max_len=2, n_candidates=4, temperature=1.15,
    ...
    Stage(1, "LARVA", "幼体 / 単語期", 14, 6, 0.0,
          GenParams(allow_fact=True, min_len=1, max_len=4, n_candidates=6, temperature=1.0,
    ...
    Stage(2, "CHILD", "幼年 / 文節期", 70, 30, 0.15,
          GenParams(allow_fact=True, min_len=2, max_len=8, n_candidates=9, temperature=0.92,
                    mutation_rate=0.10, topic_bonus=1.0, allow_mimic=True,
                    mimic_bias=0.9, mimic_len_penalty=0.06,
                    silence_threshold=-9.5),
          "短い文が形になる。話題を少し引き継げる。"),
    Stage(3, "TEEN", "少年 / 文脈期", 220, 110, 0.30,
          GenParams(allow_fact=True, min_len=3, max_len=12, n_candidates=12, temperature=0.86,
    ...
    Stage(4, "ADULT", "成体 / 対話期", 650, 320, 0.42,
          GenParams(allow_fact=True, min_len=4, max_len=18, n_candidates=14, temperature=0.82,
    ...
    Stage(5, "NAVI", "ネットナビ / 自律期", 1600, 900, 0.52,
          GenParams(allow_fact=True, min_len=4, max_len=22, n_candidates=16, temperature=0.8,
...
class Growth:
    ...
    def stage(self, vocab: int, turns: int) -> Stage:
        cur = STAGES[0]
        for st in STAGES:
            if st.unlocked(vocab, turns, self.understanding):
                cur = st
    ...
    def to_dict(self) -> dict:
        return {"coherence_ema": self.coherence_ema, "best_level": self.best_level,
                "understanding": self.understanding, "history": self.history}
